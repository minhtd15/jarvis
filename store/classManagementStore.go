...
func (c *classManagementStore) GetTableStore(documentId int, ctx context.Context) ([]qlda.Md_table, error) {
	log.Infof("Get table store")

	sqlQuery := "SELECT content, table_index FROM md_table WHERE document_id = ?"
	var mdTable []qlda.Md_table
	err := c.db.SelectContext(ctx, &mdTable, sqlQuery, documentId)

	if err != nil {
		log.WithError(err).Errorf("Failed to retrieve table from the database")
		return nil, err
	}
...
func (c *classManagementStore) GetImageStore(documentId int, ctx context.Context) ([]qlda.ImageDb, error) {
	log.Infof("Get image store")

	sqlQuery := "SELECT name, content FROM image WHERE document_id = ?"
	var mdTable []qlda.ImageDb
	err := c.db.SelectContext(ctx, &mdTable, sqlQuery, documentId)

	if err != nil {
		log.WithError(err).Errorf("Failed to retrieve image from the database")
		return nil, err
	}
